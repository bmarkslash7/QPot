...
#### Finding equilibria
Because we have an idea where the equilibria are, we create an area to find steady-state equilibria using `rootSolve::stode`.  Because it's a small space, we'll first use a `for` loop over the x- and y-area we wish to sample, then we find unique values of (x, y) to give us a matrix of unique equilibria.
```R
	xspace <- seq(1, 5, length.out = 10)
	yspace <- seq(2.5, 4, length.out = 10)
	l.xspace <- length(xspace)
	l.yspace <- length(yspace)
	space.mat <- matrix(data = NA, nrow = l.xspace*l.yspace, ncol = 2)	

	for (i in 1:l.xspace){
		for (j in 1:l.yspace){
			y <- c(x = xspace[i], y = yspace[j])
			STO <- stode(y = y, func = model.ex1, parms = NULL, positive = T)
			space.mat[(((i-1)*l.xspace)+j),] <- STO$y
		}
	}
	eqs <- unique(round(space.mat, 2))
```

#### Classifying equilibria
Once we have equilibria, we can classify them using `phaseR::stability`
```R
	stability(deriv = model.ex1, y.star = NULL, parameters = NULL, summary = TRUE)
```
