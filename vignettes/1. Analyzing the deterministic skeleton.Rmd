...
```R
	xlims <- c(0, 6)
	ylims <- c(0, 6)
	flowField(model.ex1, x.lim = xlims, y.lim = ylims, parameters = model.parms, points = 30, add = F)
...
```R
	for (i in 1:nrow(eqs)){
	print(paste0("x = ", eqs[i, 1], ", y = ", eqs[i, 2], " is a ", stability(deriv = model.ex1, y.star = eqs[i,], parameters = model.parms, summary = F)$classification))
	}
```
