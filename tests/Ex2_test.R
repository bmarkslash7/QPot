...
##### 0.5 quasi-potential vizualization!!! #####
	QPContour(e2.global,c(1000,1000), x.bound=xbounds, y.bound=ybounds,c.parm=10, xlab=expression(italic(X)), ylab = expression(italic(Y)))
...
##### 0.6 vector field decompisition!!! #####
	...
	VDV <- VecDecompVec(x.num.steps=xstepnumber, y.num.steps=ystepnumber, x.rhs=testequationx, y.rhs=testequationy, x.bound=xbounds, y.bound=ybounds)
		VecDecompPlot(field=list(VDV[,,1],VDV[,,2]), dens=c(25,25), x.bound=xbounds, y.bound=ybounds , tail.length = 0.5 , length = 0.03 , arrow.type="proportional", x.lim=c(2,6), y.lim=c(3,7), xlab=expression(italic(X)), ylab = expression(italic(Y)))

	# 0.6.2 gradient field	
	VDG <- VecDecompGrad(e2.global)
		VecDecompPlot(field=list(VDG[,,1],VDG[,,2]), dens=c(25,25), x.bound=xbounds, y.bound= ybounds , tail.length = 0.1 , length = 0.03 , arrow.type = "proportional", x.lim=c(2,6), y.lim=c(3,7), xlab=expression(italic(X)), ylab = expression(italic(Y)))
	...
	VDR <- VecDecompRem(surface=e2.global, x.rhs=testequationx, y.rhs=testequationy, x.bound=xbounds, y.bound=ybounds)
		VecDecompPlot(field=list(VDR[,,1],VDR[,,2]), dens=c(25,25), x.bound=xbounds, y.bound=xbounds , tail.length = 0.2 , length = 0.03 , arrow.type = "proportional", x.lim=c(2,6), y.lim=c(3,7), xlab=expression(italic(X)), ylab = expression(italic(Y)))
