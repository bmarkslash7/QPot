...
#'
#' Converts differential equations from string-format (or function-format) with parameters to string-format with parameter values.  Specifically, Model2String reads in the equations, searches for the differential equations within the function, and replaces the parameters with numerical values.  Returns a list of strings containing the differential equations.
#' 
#' @param model contains the differential equations as given to \code{\link{TSTraj}}.  Can either be a string or a function used by \code{deSolve}.
#' @param parms a named vector of paramters and their respective values for the deterministic equations.  If left empty, Model2String will return the equation as a string.
#' @param deSolve.form if FALSE (default) interprets model as a string containing the rhs of the equation.  If TRUE, treats model as the function input into deSolve.
...
#' @return equations a list with strings of the equations with substituted parameter values.  If deSolve.form is TRUE, the first is the x equation, the second is the y equation
#'
#' @examples
