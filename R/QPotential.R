...
#' @param save.to.R boolean to output the matrix of results for the ordered upwind method to the current R session.  The default is to write the matrix to the R session.  save.to.R=FALSE prevents the output from being written to the R session.
#' @param save.to.HD boolean to write the matrix of results for the ordered upwind method to the hard drive in a file named filename.  Default is FALSE.
...
#' @param bounce by default, the ordered upwind method stops when the boundaries are reached (x.bound and y.bound).  The bounce parameter allows the default action (bounce = 'd'), only positive values to be tested (bounce = 'p'), or reflection near the boundaries (bounce = 'b').
#' @param bounce.edge if bounce = 'b', then to prevent the ordered upwind method from reaching the boundaries, temporary boundaries are created inside the boundaries defined by x.bound and y.bound.  The boundary edge is bounce.edge of the total range.  Default is 0.01
...
QPotential <- function(x.rhs = NULL, x.start = NULL, x.bound = NULL, x.num.steps = NULL, y.rhs = NULL, y.start = NULL, y.bound = NULL, y.num.steps = NULL, filename = NULL, save.to.R = TRUE, save.to.HD = FALSE, bounce = "d", bounce.edge = 0.01, verboseR = FALSE, verboseC = TRUE, debugC = FALSE, k.x = 20, k.y = 20, INFINITY = 1000000) {
  ...
  if (!is.null(x.start) && !isTRUE(is.numeric(x.start))) {
    stop("x.start is not a number")
  }
  if (!is.null(x.bound) && !isTRUE(is.numeric(x.bound))) {
    stop("x.bound is not a number")
  }
  if (!is.null(x.num.steps) && !isTRUE(is.numeric(x.num.steps))) {
    stop("x.num.steps is not a number")
  }
  if (!is.null(y.start) && !isTRUE(is.numeric(y.start))) {
    stop("y.start is not a number")
  }
  if (!is.null(y.bound) && !isTRUE(is.numeric(y.bound))) {
    stop("y.bound is not a number")
  }
  if (!is.null(y.num.steps) && !isTRUE(is.numeric(y.num.steps))) {
    stop("y.num.steps is not a number")
  }
  if ((bounce.edge != 0.01) && !isTRUE(is.numeric(bounce.edge))) {
    stop("bounce.edge is not a number")
  }
  ...
  # ----------------------------------------------------------------------
  if (verboseR) {
    message("check if any component of x is missing")
  }
  # if (x.rhs == 'NULL') {stop("No equation defined for x. Define x.rhs.")} else {equationx <- x.rhs}
  if (is.null(x.rhs)) {
    stop("No equation defined for x. Define x.rhs.")
  } else {
    equationx <- x.rhs
  }
  if (grepl(pattern = "=", x = x.rhs)) {
    stop("Equals sign (=) found in x.rhs parameter.  Please give only right hand side of the equation.")
  }
  # if (x.start == 'NULL') {stop("No starting value for x. Define x.start.")} else {startxval <- x.start}
  if (is.null(x.start)) {
    stop("No starting value for x. Define x.start.")
  } else {
    startxval <- x.start
  }
  # if (x.num.steps == 'NULL') {stop("Need the number of steps in x range. Define x.num.steps.")} else {numofstepsx <- x.num.steps}
  if (is.null(x.num.steps)) {
    stop("Need the number of steps in x range. Define x.num.steps.")
  } else {
    numofstepsx <- x.num.steps
  }
  ...
  # ----------------------------------------------------------------------
  if (verboseR) {
    message("check if any component of y is missing")
  }
  ...
  if (grepl(pattern = "=", x = y.rhs)) {
    stop("Equals sign (=) found in y.rhs parameter.  Please give only right hand side of the equation.")
  }
  lengthequationy <- nchar(y.rhs)
  if ((length(y.bound) == 1) && (y.bound == "NULL")) {
    lowerboundsy <- 0
    upperboundsy <- 0
  }
  # if (y.bound[1] != 'NULL') {
  ...
  if (is.null(y.bound)) stop("No minimum and maximum y values.  Parameter y.bound not defined")
  # The numofstepsy cannot equal 1, because hy=(LY2-LY1)/(NY-1), where NY is numofsteps
  if (is.null(y.num.steps)) {
    numofstepsy <- 2
  } else {
    numofstepsy <- y.num.steps
  }
  if (is.null(y.start)) {
    startyval <- 0
  } else {
  ...
  if (is.null(filename) && (save.to.HD == TRUE) && ((abs(x.start) > 99999) || (abs(y.start) > 99999))) {
    stop("Cannot use default filename because program will crash.  Please supply filename")
  }

  if ((save.to.R == TRUE) && (save.to.HD == TRUE)) {
    datasave <- 3
  } else if ((save.to.R == TRUE) && (save.to.HD == FALSE)) {
    datasave <- 2
  } else if (isTRUE(save.to.HD)) {
    datasave <- 1
  } else {
    datasave <- 4
  }
  if (save.to.HD == "testrun") {
    datasave <- 4
  }
  if (save.to.HD == "original") {
    datasave <- 5
  }
  if (verboseR) {
    message("Variable datasave is: ", datasave)
  }
  ...
  # ----------------------------------------------------------------------
  if (verboseR) {
    message("Determine what C code does at edges of x.bound and y.bound")
  }
  ...
  if (bounce == FALSE) {
    ...
    bounce.style <- "d"
  } else if ((bounce == "p") || (bounce == "b")) {
    bounce.style <- bounce
  } else {
    stop("Parameter bounce must be left blank, (d)efault), (p)ositivevalues, or (b)ounce")
  }
...

  {
    ...
    if ((lowerboundsx > startxval) || (upperboundsx < startxval)) {
      stop("Starting x value x.start is outside x.bound range")
    }
    if ((lowerboundsy > startyval) || (upperboundsy < startyval)) {
      stop("Starting y value y.start is outside y.bound range")
    }
    if (lowerboundsx > upperboundsx) {
      stop("In x.bound, upper bound is less than lower bound.  x.bound[2] < x.bound[1]")
    }
    if (lowerboundsy > upperboundsy) {
      stop("In y.bound, upper bound is less than lower bounds. y.bound[2] < y.bound[1]")
    }
    ...
    if (numofstepsx * numofstepsy > 7000 * 7000) {
      ...
      if (tempsizeofarray > 1e+06) {
        tempsizeofarraystring <- paste(tempsizeofarray / 1e+06, "MB", sep = " ")
      }
      if (tempsizeofarray > 1e+09) {
        tempsizeofarraystring <- paste(tempsizeofarray / 1e+09, "GB", sep = " ")
      }
  ...
  if (datasave == 1) {
    ...
    warning("datasave is not a possible number.  How did you get here?")
  }
} # end of QPotential
