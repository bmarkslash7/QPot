...
QPContour <- function(surface, dens, x.bound, y.bound, n.filled.contour=25, n.contour.lines=25, c.parm=1, col.contour, contour.lines = TRUE, ...){
	...
	eq <- surface
	sub.x <- round(seq(1,nrow(eq),length.out=dens[1]))
	sub.y <- round(seq(1,ncol(eq),length.out=dens[2]))
	...
	x.pretty <- pretty(c((rowmin/dens[1]*x.range-min(x.bound))*0.95,(rowmax/dens[1]*x.range-min(x.bound))*1.05))
	x.pretty.at <- (x.pretty-min(y.bound))/y.range*dens[1]
	y.pretty <- pretty(c((colmin/dens[2]*y.range-min(y.bound))*0.95,(colmax/dens[2]*y.range-min(y.bound))*1.05))
	y.pretty.at <- (y.pretty-min(y.bound))/y.range*dens[2]
