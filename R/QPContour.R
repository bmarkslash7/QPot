...
QPContour <- function(surface, density, x.bound, y.bound, n.filled.contour=25, n.contour.lines=25, c=1, col.contour, contour.lines = TRUE, ...){
	...
	eq <- surface
	sub.x <- round(seq(1,ncol(eq),length.out=density[1]))
	sub.y <- round(seq(1,nrow(eq),length.out=density[2]))
	...
	x.pretty <- pretty(c((rowmin/density[1]*x.range-min(x.bound))*0.95,(rowmax/density[1]*x.range-min(x.bound))*1.05))
	x.pretty.at <- (x.pretty-min(y.bound))/y.range*density[1]
	y.pretty <- pretty(c((colmin/density[2]*y.range-min(y.bound))*0.95,(colmax/density[2]*y.range-min(y.bound))*1.05))
	y.pretty.at <- (y.pretty-min(y.bound))/y.range*density[2]

	plot(0 , type = "n" , xlim = c(rowmin,rowmax) , ylim = c(colmin,colmax) , las = 1 , xaxt = "n" , yaxt = "n" , ...)
