...
TSDensity <- function(mat, dim = 1, xlim = NULL, ylim = NULL, contour.levels = 15, col2d = c("blue", "yellow", "orange", "red"), contour.lwd = 0.5, contour.lines = TRUE, kde2d.n = 100, xlab = "X", ylab = "Y", ...) {
  if (dim == 1) {
    ...
    if (missing(xlim)) {
      xlim <- c(min(c(densA$x, densB$x)), max(c(densA$x, densB$x)))
    }
    if (missing(ylim)) {
      ylim <- c(min(c(densA$y, densB$y)), max(c(densA$y, densB$y)))
    }
  ...
  if (dim == 2) {
    ...
    if (contour.lines == T) {
      contour(x.range, y.range, kern.2d$z, levels = contour.breaks, drawlabels = F, add = TRUE, col = "black", lwd = contour.lwd)
    }

    par(new = TRUE)
    if (missing(xlim) != missing(ylim)) {
      stop("Both xlim and ylim must be specified, or neither")
    }
    if (missing(xlim)) {
      xlim <- c(min(kern.2d$x), max(kern.2d$x))
    }
    if (missing(ylim)) {
      ylim <- c(min(kern.2d$y), max(kern.2d$y))
    }
