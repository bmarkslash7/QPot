...
QPGlobal <- function(local.surfaces, unstable.eq.x, unstable.eq.y, x.bound, y.bound) {
  ...
  if (n.unstable.eq.x != n.unstable.eq.y) {
    stop("Unstable x and y points not equal")
  }
  ...
  if (nrow(max.phi.arr) != n.unstable.pts) {
    # if not all max(phi) are the same, then they need to be aligned
